...
- [ ] 1.3 Verify migration upgrade/downgrade paths are valid.
...
- [ ] 4.6 Confirm `/start` and `/restart` continue to exclude previous-thread history through the existing thread reset mechanism.

## 5. Provider Coverage

- [ ] 5.1 Add/adjust tests proving OpenAI requests receive history through the common message-building path.
- [ ] 5.2 Add/adjust tests proving Anthropic requests receive history through the common message-building path.
- [ ] 5.3 Add/adjust tests proving Mistral requests receive history through the common message-building path.
- [ ] 5.4 Add/adjust tests proving the censor LLM request receives recent history when censor is enabled.

## 6. Regression Tests

- [ ] 6.1 Add admin settings tests for rendering, saving, validation errors, default value, and audit metadata for `LLM_HISTORY_MESSAGES`.
- [ ] 6.2 Add history service tests for ordering, limit behavior, zero-history behavior, and thread isolation.
- [ ] 6.3 Add message handler tests proving final responses are persisted once and failed responses are not persisted.
- [ ] 6.4 Run the full test suite and fix regressions.
