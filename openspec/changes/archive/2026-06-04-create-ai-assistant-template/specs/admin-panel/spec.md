...
- AND SHALL listen on port `8080` internally

### Requirement: Admin panel shall provide MVP sections
...
- GIVEN an authenticated admin opens the admin panel
- WHEN the dashboard loads
- THEN navigation links SHALL be present for System Prompt, Tools Instruction, Censor, Administrators and Debug sections
...
- WHEN admin opens the System Prompt page
...
- WHEN admin edits and saves the system prompt
...
- GIVEN admin has `superadmin` role
- WHEN admin submits a new admin with Telegram ID and role
- THEN the system SHALL create the admin record

#### Scenario: Non-superadmin views admins

- GIVEN admin has `read` or `write` role
- WHEN admin opens the Administrators page
- THEN the admin list SHALL be visible
- AND add/deactivate/role-change controls SHALL NOT be available
...
- GIVEN an authenticated admin requests an admin page
- WHEN the request reaches the admin service
...
- optional preview/test area (stretch goal).

#### Scenario: Admin enables censor

- GIVEN admin has `write` or `superadmin` role
- WHEN admin toggles censor enabled checkbox and saves
- THEN censor SHALL be enabled for all subsequent user messages
- AND the setting SHALL be persisted in DB settings table

#### Scenario: Admin disables censor

- GIVEN censor is currently enabled
- WHEN admin toggles censor disabled and saves
- THEN censor LLM pass SHALL be skipped for all subsequent messages
- AND draft responses SHALL be sent directly to users

#### Scenario: Admin saves censor prompt

- GIVEN admin has `write` or `superadmin` role
- WHEN admin edits and saves censor prompt
- THEN system SHALL create a new censor prompt version
...
- GIVEN admin has `write` or `superadmin` role
- WHEN admin clicks restore on one of the last three censor prompt versions
...
- AND no secret values SHALL be visible
