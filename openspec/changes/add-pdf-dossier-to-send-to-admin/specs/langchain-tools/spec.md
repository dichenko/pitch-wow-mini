...
After sending the markdown history file, the tool SHALL attempt to generate a PDF founder dossier from the user's full dialogue history and send the PDF document to the admin chat. PDF dossier generation SHALL be best-effort: failure SHALL be reported to the admin chat and persisted in notification payload metadata, but SHALL NOT undo the main notification or markdown history delivery.
...
- **THEN** the tool SHALL attempt to send a generated PDF founder dossier
...
- **THEN** the tool SHALL attempt to send a generated PDF founder dossier attachment
- **THEN** also save the notification record to `admin_notifications` with `delivered = TRUE`

#### Scenario: PDF dossier generation succeeds

- **WHEN** `ADMIN_TELEGRAM_CHAT_ID` is set and PDF dossier generation returns a downloaded PDF file
- **THEN** `send_to_admin` SHALL send the PDF file to the admin chat via `send_document`
- **THEN** the tool SHALL delete the temporary PDF file after sending or after a send failure
- **THEN** the notification payload SHALL include PDF dossier metadata

#### Scenario: PDF dossier generation fails

- **WHEN** `ADMIN_TELEGRAM_CHAT_ID` is set and PDF dossier generation fails
- **THEN** `send_to_admin` SHALL send an admin-chat message stating that the PDF was not created
- **THEN** the main notification and markdown history delivery SHALL remain successful if they already completed
- **THEN** the notification payload SHALL include the PDF generation error metadata
